...
from precise.automatic_training.utils import numConcat
...
class Augmenter:
    """
        ...
        an specified folder. Also crudely measure and print execution time.
    """
    def __init__(self, file_label, wakeword_amount):
        CURRENT_DIR = os.path.dirname(__file__)
        MAIN_DIR = os.path.dirname(CURRENT_DIR)
        WAKE_WORD_DIR = file_label + "/wake-word"
        self.output_dir = os.path.join(MAIN_DIR, WAKE_WORD_DIR)
        os.makedirs(self.output_dir, exist_ok=True)

        np.random.seed(42)
        random.seed(42)

        # better to use wakeword_amount up to 9
        label_numbers = range(0, wakeword_amount)
        # labels = [numConcat(0, number) for number in label_numbers]

        self.sound_file_paths = [
            Path(os.path.join(WAKE_WORD_DIR, file_label + "-" + numConcat(0, number) + ".wav")) for number in label_numbers
        ]
        if all([os.path.exists(path) for path in self.sound_file_paths]):
            pass
        else:
            raise Exception("No specified amount of audio files!")
        self.transforms = [
                {
                    "instance": AddBackgroundNoise(
                        sounds_path=os.path.join(CURRENT_DIR, "background_noises"), p=1.0
                    ),
                    "num_runs": 5,
                },
                {
                ...
                {
                    "instance": AddImpulseResponse(p=1.0, ir_path=os.path.join(CURRENT_DIR, "ir")),
                    "num_runs": 1,
                },
                {
                    "instance": AddImpulseResponse(
                        p=1.0, ir_path=os.path.join(CURRENT_DIR, "ir"), leave_length_unchanged=True
                    ),
                    "num_runs": 1,
                ...
                },
                {
                    "instance": AddShortNoises(
                        sounds_path=os.path.join(CURRENT_DIR, "short_noises"),
                        min_snr_in_db=0,
                        max_snr_in_db=8,
    ...
    def augmentate(self):
        for sound_file_path in self.sound_file_paths:
...
if __name__ == "__main__":
    augmenter = Augmenter('hey-hakan', wakeword_amount=5)
    # augmenter.augmentate()